...
export default function RegistrationForm({ selectedTicketType, setSelectedTicketType, setView }: RegistrationFormProps) {
  ...
  const getInstallmentOptions = (packagePrice: number) => {
    const list = [5, 30, 55, 65, 105, 185, 205, 255, 355];
  ...
  const completeBooking = async (bookingReference: string, paymentType: string) => {
    let bookingToSave: BookingConfirmation;
    const currentTicketType = existingBooking ? existingBooking.registration.ticketType : formData.ticketType;
    const pkgPrice = TICKET_PACKAGES.find(p => p.type === currentTicketType)?.price || 100;

    if (existingBooking) {
      const oldPaid = existingBooking.amountPaid !== undefined ? existingBooking.amountPaid : pkgPrice;
      const newTotalPaid = oldPaid + selectedPayment;
      ...
      bookingToSave = {
        ...
        registration: {
          ...existingBooking.registration,
          fullName: formData.fullName || existingBooking.registration.fullName,
          phone: formData.phone || existingBooking.registration.phone,
        }
      };

      setTallyNotice({
        isNew: false,
        addedPayment: selectedPayment,
      ...
      bookingToSave = {
        ...
        status: selectedPayment >= selectedPkg.price ? 'Confirmed' : 'Pending',
        paymentRef: bookingReference,
        paymentMethod: paymentType
      };
      ...
      setTallyNotice({
        ...
        addedPayment: selectedPayment,
        previousPaid: 0,
        totalPaid: selectedPayment,
        fullPrice: selectedPkg.price
      });
    }
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    // Generate unique transaction reference for the checkout session
    const ref = 'DN-' + Math.floor(1000000 + Math.random() * 9000000);

    // Persist pending registration so we can resume after Paystack redirect
    try {
      sessionStorage.setItem('wcf_pending_registration', JSON.stringify(formData));
      sessionStorage.setItem('wcf_pending_payment', String(selectedPayment));
      sessionStorage.setItem('wcf_pending_ref', ref);
    } catch (err) {
      console.warn('Could not persist pending registration to sessionStorage', err);
    }

    // Initialize Paystack transaction on the server and redirect to authorization_url
    try {
      const resp = await fetch('/api/paystack/initialize', {
        ...
        body: JSON.stringify({
          ...
          amount: Math.round(selectedPayment * 100), // amount in pesewas (GHS)
          reference: ref,
          currency: 'GHS'
        })
      });
      const json = await resp.json();
      if (resp.ok && json.data && json.data.authorization_url) {
        // Redirect user to Paystack hosted payment page
        window.location.href = json.data.authorization_url;
        return;
      } else {
        console.error('Paystack initialize failed', json);
        // Fallback: complete booking locally
        await completeBooking(ref, 'Direct Registration');
        return;
      }
    } catch (err) {
      console.error('Paystack init request failed', err);
      await completeBooking(ref, 'Direct Registration');
      return;
    }
  };

  // If user is redirected back from Paystack with a `reference` query param,
  // verify the transaction on the server and complete the booking.
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const pending = sessionStorage.getItem('wcf_pending_registration');
        const pendingPayment = sessionStorage.getItem('wcf_pending_payment');
        if (pending) {
          try {
            const parsed = JSON.parse(pending);
            setFormData(prev => ({ ...prev, ...parsed }));
          } catch (e) {
            console.warn('Failed to parse pending registration', e);
          }
        }
        if (pendingPayment) setSelectedPayment(Number(pendingPayment));
      } catch (e) {
        console.warn('Could not read pending registration from sessionStorage', e);
      }

      try {
        const resp = await fetch(`/api/paystack/verify/${encodeURIComponent(reference)}`);
        const json = await resp.json();
        if (resp.ok && json.data && json.data.status === 'success') {
          // Completed payment — finalize booking
          await completeBooking(reference, 'Paystack');
          // Clear any pending items
          sessionStorage.removeItem('wcf_pending_registration');
          sessionStorage.removeItem('wcf_pending_payment');
          sessionStorage.removeItem('wcf_pending_ref');
          // remove reference from URL to keep UX clean
      ...
      <section className="min-h-screen pt-32 pb-24 px-6 md:px-12 bg-obsidian flex flex-col justify-center items-center relative overflow-hidden">
        ...
        <div className="w-full max-w-xl text-center relative z-10 animate-fade-in-up">
          ...
          <p className="text-base text-pearl/80 font-light leading-relaxed mb-8">
            Your reservation for the <span className="text-sand font-semibold font-serif">WCF-KNUST Dinner Night '26</span> is successfully cataloged. We look forward to hosting you for this unforgettable evening of elegance, refined fellowship, and divine connection.
          </p>
    ...
    {showSuccessOverlay && confirmedBooking && (
      <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-4 overflow-y-auto no-print">
        ...
        >
          ...
          <div className="bg-[#240108]/70 border border-[#B38F6F]/25 rounded-xl p-6 text-left space-y-4 mb-8 backdrop-blur-sm">
            ...
            <div className="border-t border-sand/10 pt-3">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-[#3ac58a] animate-pulse" />
                <p className="text-[9px] font-mono text-[#3ac58a] uppercase tracking-wider">
                  Registration Reference: {confirmedBooking.paymentRef || 'N/A'}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            <span>View My Ticket Details</span>
